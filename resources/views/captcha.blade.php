...
<fieldset class="form-label-group form-group position-relative has-icon-left" id="captcha">
    <input
        ...
        placeholder="{{ Guanguans\FilamentLoginCaptcha\LoginCaptchaServiceProvider::trans('login-captcha.captcha') }}"
    ...
    <span style="width: 37%;height: 33.5px;position: absolute;top: 0;right: 0;border-radius: .25rem;border: 1px solid #dbe3e6;">
        <img
            alt="{{ Guanguans\FilamentLoginCaptcha\LoginCaptchaServiceProvider::trans('login-captcha.captcha') }}"
            id="captcha-img"
            src="{{ login_captcha_url() }}"
            style="cursor: pointer;width: 100%;height: 100%;border-radius: .25rem;"
            title="{{ Guanguans\FilamentLoginCaptcha\LoginCaptchaServiceProvider::trans('login-captcha.refresh_captcha') }}"
        >
    </span>
    <div class="form-control-position"><i class="feather icon-image"></i></div>
    <label for="captcha-input">{{ Guanguans\FilamentLoginCaptcha\LoginCaptchaServiceProvider::trans('login-captcha.captcha') }}</label>
